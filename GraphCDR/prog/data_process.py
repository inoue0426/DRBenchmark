...
def process(
    ...
    cellline_num = np.squeeze(
        [[j[1] for j in cellmap if i[0] == j[0]] for i in data_new]
    )
    pubmed_num = np.squeeze(
        [[j[1] for j in pubmedmap if i[1] == j[0]] for i in data_new]
    )
    ...
    train_mask = coo_matrix(
        (np.ones(train.shape[0], dtype=bool), (train[:, 0], train[:, 1])),
        shape=(nb_celllines, nb_drugs),
    ).toarray()
    test_mask = coo_matrix(
        (np.ones(test.shape[0], dtype=bool), (test[:, 0], test[:, 1])),
        shape=(nb_celllines, nb_drugs),
    ).toarray()
