...
from torch_geometric.nn import GatedGraphConv, SGConv
from utils import *
...
class SubCDR(nn.Module):
    ...
    def interaction_graph(self, maps):
        ...
        graph_batch = Data.DataLoader(
            dataset=GraphDataset(graphs_dict=graphs),
            collate_fn=collate,
    ...
    def feature_initialize(self, node_num, max_dim):
        ...
        node_feat = (
            torch.cat((node_feat, torch.zeros([node_num, max_dim - node_num])), axis=1)
            if (node_num < max_dim)
            else node_feat
        )
        return node_feat
