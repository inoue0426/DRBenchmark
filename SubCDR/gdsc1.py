...
def train_and_validate_fold(train_set, validation_set, args):
    ...
    # Setup model
    model, optimizer, loss_fn = setup_model(drug_dim, glo_dim, device, args)
    ...
    for epoch in range(args.ep):
        ...
        train_epoch(model, train_loaders, optimizer, loss_fn)
        auc, aupr, y_true, y_pred = validate(model, valid_loaders, loss_fn)
...
def run_cross_validation(args):
    ...
    for train_index, test_index in kfold.split(np.arange(pos_num)):
        ...
        best_pred, y_true = train_and_validate_fold(train, test, args)
