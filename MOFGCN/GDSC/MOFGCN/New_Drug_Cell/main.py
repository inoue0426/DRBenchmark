...
for dim in target_dim:
    for target_index in np.arange(cell_drug.shape[dim]):
        ...
        if dim:
            ...
            true_data_s.to_csv(
                "./result_data/drug_" + str(target_index) + "_true_data.csv"
            )
            predict_data_s.to_csv(
                "./result_data/drug_" + str(target_index) + "_predict_data.csv"
            )
        else:
            file_cell.write(str(target_index) + ":" + str(epochs) + "\n")
            true_data_s.to_csv(
                "./result_data/cell_" + str(target_index) + "_true_data.csv"
            )
            predict_data_s.to_csv(
                "./result_data/cell_" + str(target_index) + "_predict_data.csv"
            )
file_drug.close()
file_cell.close()
