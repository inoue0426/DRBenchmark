...
class CsvLimitSizeZip(object):
    ...
    def _uzip_file(path_dir: str, file_names: list):
        ...
        datas = [
            CsvLimitSizeZip.__uzip_file(path_dir=path_dir, file_name=name)
    ...
    def split_file(self):
        ...
        if file_size > self.size:
            ...
            fun_map = {
                0: CsvLimitSizeZip.__split_row_data,
                1: CsvLimitSizeZip.__split_col_data,
            }
            datas = fun_map[dim](data=data, number=split_number)
            paths = [
                path + "_dim_{}_num_{}.csv".format(dim, i) for i in range(split_number)
            ]
