...
class GEncoder(nn.Module, ABC):
    ...
    def forward(self):
        ...
        cell_emb = fun.relu(
            self.lm_cell((1 - self.alpha) * cell_gcn + self.alpha * cell_ni)
        )
        drug_emb = fun.relu(
            self.lm_drug((1 - self.alpha) * drug_gcn + self.alpha * drug_ni)
        )
...
class nihgcn(nn.Module, ABC):
    def __init__(
        self, adj_mat, cell_exprs, drug_finger, layer_size, alpha, gamma, device="cpu"
    ):
